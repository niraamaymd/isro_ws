    ...
    def mission(self):
        ...
        while not self.stop_mission:
            if self.is_vision_pose_healthy:
                ...
                rospy.loginfo("Applying small pitch to drive forward")
                pitch_msg = OverrideRCIn()
                pitch_msg.channels = [0] * 18
                pitch_msg.channels[2] = 1640
                pitch_msg.channels[1] = 1470  # Small forward pitch (1500 is neutral, lower = forward)
                
                # Apply pitch for a short duration
                pitch_start_time = rospy.get_time()
                pitch_duration = 10  # 2 seconds of forward movement
                rate = rospy.Rate(10)
                ...
                rospy.loginfo("Setting to FLOW_HOLD for 5 seconds")
                self.set_mode("22")  # FLOW_HOLD
                rospy.sleep(5)
