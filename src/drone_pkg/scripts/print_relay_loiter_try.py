...
class DroneController(object):
    def __init__(self):
        ...
        self.print_pub = rospy.Publisher("/print_relay", String, queue_size=10)
    ...
    def manual_cmd_cb(self, msg):
        ...
        self.print_relay("[MANUAL CMD] Received: '{}'".format(cmd))
        if cmd == "arm":
            self.arm_drone()
        elif cmd == "disarm":
            self.disarm_drone()
            ...
            if len(parts) == 2:
                ...
                self.print_relay("Usage: 'mode <MODE>'")
        elif cmd in ("land", "l"):
            ...
            self.print_relay("Unknown manual command: '{}'".format(cmd))

    def print_relay(self, data):
        self.print_relay(data)
        msg = String
        msg.data = data
    ...
    def pose_cb(self, msg):
        ...
        if pose.position.x == pose.position.y == pose.position.z == 0.0000 and \
           pose.orientation.x == pose.orientation.y == pose.orientation.z == pose.orientation.w == 0.000:
            self.is_vision_pose_healthy = False
            self.print_relay("VISION POSE UNHEALTHY!")
        ...
        if self.print_pose:
            self.print_relay("Pose: {}, {}, {}".format(pose.position.x, pose.position.y, pose.position.z))

    def set_ekf_origin(self):
        self.print_relay("Publishing EKF origin via topic...")
        ...
        if origin_pub.get_num_connections() == 0:
            self.print_relay("No subscribers connected to /mavros/global_position/set_gp_origin. "
                          "The message may not be received.")

        try:
            origin_pub.publish(geo_point_msg)
            self.print_relay("EKF origin published: (lat: %f, lon: %f, alt: %f)",
                          geo_point_msg.position.latitude,
                          geo_point_msg.position.longitude,
                          geo_point_msg.position.altitude)
        except rospy.ROSException as e:
            self.print_relay("Failed to publish EKF origin: %s", e)
    ...
    def arm_drone(self):
        ...
        try:
            ...
            if resp.success:
                self.print_relay("Arming succeeded.")
            else:
                self.print_relay("Arming failed. (Service call succeeded)")
        except rospy.ServiceException as e:
            self.print_relay("Arming call failed. (err: %s)", e)
    ...
    def disarm_drone(self):
        ...
        try:
            ...
            if resp.success:
                self.print_relay("Disarming succeeded.")
            else:
                self.print_relay("Disarming failed. (Service call succeeded)")
        except rospy.ServiceException as e:
            self.print_relay("Disarming call failed. (err: %s)", e)
    ...
    def takeoff(self):
        ...
        while True:
            ...
            if rospy.get_time() - self.takeoff_start_time >= self.takeoff_duration and not resetted_once_check:
                self.print_relay("TAKEOFF Completed.")
                self.set_ekf_origin()
                self.reset_odom()
                self.resume_odom()
                self.print_relay("Waiting for vision pose")
    ...
    def pause_odom(self):
        try:
            ...
            self.print_relay("Odometry paused.")
        except (rospy.ROSException, rospy.ServiceException) as e:
            self.print_relay("Odometry pause call failed. (err: %s)", e)
    ...
    def reset_odom(self):
        try:
            ...
            self.print_relay("Odometry resetted.")
        except (rospy.ROSException, rospy.ServiceException) as e:
            self.print_relay("Odometry reset call failed. (err: %s)", e)
    ...
    def reset_odom_with_last_pose(self):
        ...
        try:
            ...
            response = reset_odom(pose.position.x,
                             ...
                             pose.position.z,
                             roll, pitch, yaw)
            self.print_relay("Odometry reset to pose {}".format(pose))
            return response
        except rospy.ServiceException as e:
            self.print_relay("Service call failed: %s" % e)
    ...
    def resume_odom(self):
        try:
            ...
            self.print_relay("Odometry resumed.")
        except (rospy.ROSException, rospy.ServiceException) as e:
            self.print_relay("Odometry resume call failed. (err: %s)", e)


    def velocity_control(self, vx, vy, vz, movt_duration=10):
        self.print_relay("Starting velocity control [x: {}, y: {}, z: {}, t: {}]".format(vx,vy,vz,movt_duration))
        ...
        self.print_relay("Finished velocity control.")
        

    def mission(self):
        self.mission_status = "STARTED"
        self.print_relay("STARTED MISSION")
        ...
        while not self.stop_mission:
            if self.is_vision_pose_healthy:
                ...
                # Use small pitch to drive forward
                self.print_relay("Applying small pitch to drive forward")
                pitch_msg = OverrideRCIn()
                pitch_msg.channels = [0] * 18
                pitch_msg.channels[2] = 1500
                pitch_msg.channels[1] = 1450  # Small forward pitch (1500 is neutral, lower = forward)
                
                # Apply pitch for a short duration
                pitch_start_time = rospy.get_time()
                pitch_duration = 2  # 2 seconds of forward movement
                rate = rospy.Rate(10)
                
                while rospy.get_time() - pitch_start_time < pitch_duration:
                    self.rc_override_pub.publish(pitch_msg)
                    rate.sleep()
                
                # Set to flow hold for 5 seconds
                self.print_relay("Setting to FLOW_HOLD for 5 seconds")
                self.set_mode("22")  # FLOW_HOLD
                rospy.sleep(5)
            ...
            #else:
                #self.print_relay("VISION POSE NOT HEALTHY")
                #self.set_mode("22")
                #self.land_immediately()
                #self.stop_mission = True
    ...
    def set_mode(self, mode):
        ...
        try:
            ...
            if resp.mode_sent:
                self.print_relay("Mode set to [%s] successfully.", mode.upper())
            else:
                self.print_relay("SetMode failed. (Service call succeeded)")

        except rospy.ServiceException as e:
            rospy.loggerr("SetMode call failed (e: %s)", e)

    def land_immediately(self):
        self.print_relay("LANDING NOW!")
        self.set_mode("LAND")
